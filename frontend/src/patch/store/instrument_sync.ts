...
export function make_in_node(instrument: InstrumentState): Node {
    return {
        ...
        type: 'in',
        position: { x: 0, y: 0 },
        data: { name: instrument.id, num_channels: out_channels_of(instrument) } satisfies InData,
    };
}
...
export function sync_global_in_nodes(instruments: InstrumentState[], global_nodes: Node[]): Node[] {
    ...
    const updated = kept.map((n) => {
        ...
        if (data.num_channels === num_channels && data.name === instrument.id) return n;
        return { ...n, data: { ...n.data, num_channels, name: instrument.id } };
    });
