...
# STAGED_ELM-FRONTEND=$(git ls-files | grep -E '^elm-frontend/.*\.(ts|html|scss|css|json)$' || true)
STAGED_ELM_FRONTEND=$(git diff --cached --name-only --diff-filter=ACMR | { grep -E '^elm-frontend/.*\.(tsx|ts|html|scss|css|json)$' || true; })

if [ -n "$STAGED_ELM_FRONTEND" ]; then
    ELM_FRONTEND_DIR="$ROOT/elm-frontend"
    ELM_FRONTEND_FILES=$(printf '%s\n' "$STAGED_ELM_FRONTEND" | sed "s|^|$ROOT/|")
    
    if ! (cd "$ELM_FRONTEND_DIR" && echo "$ELM_FRONTEND_FILES" | xargs npx prettier --write) 2>&1; then
        echo "  prettier: formatting failed"
        exit 1
    fi
